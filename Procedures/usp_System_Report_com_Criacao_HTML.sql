...
USE [master]
GO
IF OBJECT_ID ('[dbo].[usp_SSAJ_SQL_Server_System_Report]') IS NOT NULL
BEGIN
	DROP PROCEDURE [dbo].[usp_SSAJ_SQL_Server_System_Report]
END
GO
...
-- Called From: [master].[dbo].[usp_SSAJ_SQL_Server_System_Report]
-- Modifications: NONE

----------------------------------------------------------------------------------------------------------------------
--	Main Query: Create Procedure
----------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE [dbo].[usp_SSAJ_SQL_Server_System_Report]
	@LocalSalvarArquivo varchar(3000)
AS
...
DECLARE @CaminhoSalvar		varchar(300)
DECLARE @Exist				BIT
-------------------------------------------
--SET @vRecipients					= 'rodrigo.barbosa@kronossolucoes.com.br'
--SET @vCopy_Recipients				= NULL
SET @vUnused_Index_Uptime_Threshold = 7
SET @CaminhoSalvar					= CASE 
										WHEN CHARINDEX('\',RIGHT(RTRIM(@LocalSalvarArquivo),1)) = 1 THEN
											@LocalSalvarArquivo+ @@SERVERNAME +'_'+ CONVERT(CHAR(8),GETDATE(),112) + '.HTML'
										ELSE
											@LocalSalvarArquivo+'\'+ @@SERVERNAME +'_'+ CONVERT(CHAR(8),GETDATE(),112) + '.HTML'
										END
-------------------------------------------
...
PRINT '		Deletar e cria o arquivo com scripts novamente'
--Verifica se existe o arquivo para deletar e cria-lo novamente
EXECUTE @RES = sp_OAMethod @FSO, 'FileExists', @Exist OUT, @CaminhoSalvar
...
IF @Exist = 1 BEGIN
	...
	Set @CMD = 'DEL "'+@CaminhoSalvar+'"'
	EXEC XP_CMDSHELL @CMD
END 

PRINT '		Abertura do arquivo para escrita'
-- Abertura do Arquivo
EXECUTE @RES = sp_OAMethod @FSO, 'OpenTextFile', @FID OUT, @CaminhoSalvar, 8, 1
...
SET @vBody ='
	...
	'
EXEC @RES = sp_OAMethod @FID, 'WriteLine', NULL, @vBody
...
SET @vBody ='
	...
	'
SET @vBody = @vBody+@vXML_String+'</table></center>'
	...
	CONVERT (NVARCHAR (MAX),
		(
			SELECT
				...
				,'','right_align'+CAST(CONVERT (MONEY, X.free_space_mb) AS VARCHAR) AS 'td'
			FROM
...
BEGIN
	SET @vSQL_String ='
			...
			SELECT
				...
				,CAST( ((A.total_size*CONVERT (BIGINT, 8192))/1048576.0) AS MONEY) AS total_size_mb
				--,(CASE
				...
				,(CASE
					WHEN A.database_size >= B.total_pages THEN CAST( (((A.database_size-B.total_pages)*CONVERT (BIGINT, 8192))/1048576.0) AS MONEY)
					ELSE ''0''
					END) AS unallocated_mb
				...
				,CAST( ((B.total_pages*CONVERT (BIGINT, 8192))/1048576.0) AS MONEY) AS reserved_mb
				...
				,CAST( ((B.pages*CONVERT (BIGINT, 8192))/1048576.0) AS MONEY) AS data_mb
				...
				,CAST( (((B.used_pages-B.pages)*CONVERT (BIGINT, 8192))/1048576.0) AS MONEY) AS index_mb
				...
				,CAST( (((B.total_pages-B.used_pages)*CONVERT (BIGINT, 8192))/1048576.0) AS MONEY) AS unused_mb
			FROM
...
----------------------------------------------------------------------------------------------------------------------
EXEC @RES = sp_OAMethod @FID, 'WriteLine', NULL, '</body></html>'
